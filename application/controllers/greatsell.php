...
class Greatsell extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('greatsell_model', '', TRUE);
    }
    ...
    public function index() {
        ...
        $this->data['ordens'] = $this->greatsell_model->getOsAbertas();
        $this->data['produtos'] = $this->greatsell_model->getProdutosMinimo();
        $this->data['os'] = $this->greatsell_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->greatsell_model->getEstatisticasFinanceiro();
        $this->data['menuPainel'] = 'Painel';
        $this->data['cliente_count'] = $this->clientes_model->count('clientes');
        $this->data['produto_count'] = $this->produtos_model->count('produtos');
        $this->data['os_count'] = $this->os_model->count('os');
        $this->data['servico_count'] = $this->servicos_model->count('servicos');
        $this->data['view'] = 'greatsell/painel';
    ...
    public function minhaConta() {
        ...
        $this->data['usuario'] = $this->greatsell_model->getById($this->session->userdata('id'));
        $this->data['view'] = 'greatsell/minhaConta';
        $this->load->view('tema/topo', $this->data);
    }
    ...
    public function alterarSenha() {
        ...
        $result = $this->greatsell_model->alterarSenha($senha, $oldSenha, $this->session->userdata('id'));
		if ($result) {
            $this->session->set_flashdata('success', 'Senha Alterada com sucesso!');
            redirect(base_url() . 'index.php/greatsell/minhaConta');
        } else {
    ...
    public function pesquisar() {
        ...
        $data['results'] = $this->greatsell_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'greatsell/pesquisa';
        $this->load->view('tema/topo', $this->data);
    }

    public function login() {
        $this->load->view('greatsell/login');
    }
    ...
    public function verificarLogin() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (count($usuario) > 0) {
                $dados = array('nome' => $usuario->nome, 'id' => $usuario->idUsuarios, 'permissao' => $usuario->permissoes_id, 'logado' => TRUE, 'entidade' => $usuario->entidade);
    ...
    public function emitente() {
        ...
        $data['dados'] = $this->greatsell_model->getEmitente();
        $data['view'] = 'greatsell/emitente';
    ...
    public function cadastrarEmitente() {
        if ((!$this->session->userdata('session_id')) || (!$this->session->userdata('logado'))) {
            redirect('index.php/greatsell/login');
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(base_url() . 'index.php/greatsell/emitente');
        } else {
            ...
            $retorno = $this->greatsell_model->addEmitente($nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {

                $this->session->set_flashdata('success', 'Registro cadastrado com sucesso.');
                redirect(base_url() . 'index.php/greatsell/emitente');
            } else {
                $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar inserir o registro.');
                redirect(base_url() . 'index.php/greatsell/emitente');
            }
        }
    }
    ...
    public function editarEmitente() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->greatsell_model->editEmitente($id, $nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email);
            if ($retorno) {
    ...
    public function editarLogo() {
        ...
        $retorno = $this->greatsell_model->editLogo($id, $logo);
        if ($retorno) {
