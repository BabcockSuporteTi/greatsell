...
class Servicos extends CI_Controller {
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('servicos') == false) {
            ...
            $preco = $this->parser->moeda($this->input->post('preco'));
    ...
    function editar() {
        ...
        if ($this->data['result'] == null) {
            $this->session->set_flashdata('error', 'Serviço "' . $this->uri->segment(3) . '" não encontrado.');
    ...
    function excluir() {
		...
		if (!empty($this->servicos_model->getServicosEmUsoOrdemServico($id))){
            $this->session->set_flashdata('error', 'Erro ao excluir. Serviço está em uso em uma ou mais ordens de serviço.');
            redirect(base_url() . 'index.php/servicos/gerenciar/');		
		}	
