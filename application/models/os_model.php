...
class Os_model extends CI_Model {
    ...
    function getById($id) {
        $entidade = $this->session->userdata('entidade');
        $this->db->select('os.*, clientes.*, usuarios.telefone, usuarios.email,usuarios.nome, clientes.telefone as clienteTelefone');
        $this->db->from('os');
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
    ...
    public function autoCompleteProduto($q) {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['descricao'] . ' -- Saldo: ' . $row['estoque'], 'estoque' => $row['estoque'], 'id' => $row['idProdutos'], 'preco' => $row['precoVenda']);
            }
            echo json_encode($row_set);
        } else {
            $row_set[] = array('label' => "Nenhum registro encontrado", 'id' => "-1");
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteCliente($q) {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['nomeCliente'] . ' - ' . $row['telefone'], 'id' => $row['idClientes']);
            }
            echo json_encode($row_set);
        } else {
            $row_set[] = array('label' => "Nenhum registro encontrado", 'id' => "-1");
    ...
    public function autoCompleteUsuario($q) {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['nome'] . ' - ' . $row['telefone'], 'id' => $row['idUsuarios']);
            }
            echo json_encode($row_set);
        } else {
            $row_set[] = array('label' => "Nenhum registro encontrado", 'id' => "-1");
    ...
    public function autoCompleteServico($q) {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['nome'], 'id' => $row['idServicos'], 'preco' => $row['preco']);
            }
            echo json_encode($row_set);
        } else {
            $row_set[] = array('label' => "Nenhum registro encontrado", 'id' => "-1");
